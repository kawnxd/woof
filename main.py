...
def start_service_and_wait(service_name):
    try:
        subprocess.run(
            ...
            check=True,
            creationflags=subprocess.CREATE_NO_WINDOW  # Komutu arka planda çalıştırır
        )
...
def main():
    ...
    if service_started:
        ...
        while True:
            clear_screen()
            print(Fore.CYAN + Style.BRIGHT + "KAWNDEV - VGC Bypass v1.0" + Style.RESET_ALL)
            print(Fore.YELLOW + "Kullanım talimatları:" + Style.RESET_ALL)

            print(Fore.GREEN + "Aşama 1" + Style.RESET_ALL)
            print(Fore.GREEN + "Aşama 2" + Style.RESET_ALL)

            print(Fore.MAGENTA + "Choose an option:" + Style.RESET_ALL)
            print(Fore.RED + "1. Run First" + Style.RESET_ALL)
            print(Fore.RED + "2. Run Second" + Style.RESET_ALL)
